  ...
  if (options.overrideMethods) {
    ...
    if (finalList.indexOf("aggregate") > -1) {
      schema.pre("aggregate", function () {
        ...
        var hasDeletedMatchStage =
          typeof firstMatch.$match?.deleted !== "undefined";
        var shouldShowAllDocuments =
          firstMatch.$match?.showAllDocuments === true;
    ...
    finalList.forEach(function (method) {
      if (
        ["count", "countDocuments", "find", "findOne", "distinct"].indexOf(
          method
        ) > -1
      ) {
